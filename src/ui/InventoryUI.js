...
export class InventoryUI {
    ...
    _buildCombatStats(member) {
        ...
        if (member.classId === 'barbarian' && member.level >= BARBARIAN_WEREBEAR_UNLOCK_LEVEL) {
            ...
            perkDetails.push(`Barbarian L${BARBARIAN_WEREBEAR_UNLOCK_LEVEL} Werebear:\n  Once per combat. +${hpPct}% max/current HP, ${Math.round(BARBARIAN_WEREBEAR_REGEN * 100)}% max HP regen per round, weapon riders suppressed while active.\n  Costs ${BARBARIAN_WEREBEAR_STAMINA_COST} ST to activate and ${BARBARIAN_WEREBEAR_STAMINA_PER_ROUND} ST each round to sustain. Adds +2% Blood Frenzy damage per bleed stack.${hasFurLoincloth ? `\n  Fur Loincloth also raises Rage regen to ${rageRegenPct}%, adds +1 rage strike, and adds +2% Blood Frenzy damage per bleed stack.` : ''}${hasClawsCavebear ? `\n  Claws of the Cavebear add +10% damage, +1 rage strike, and +2% Blood Frenzy damage per bleed stack per claw while in Werebear form.` : ''}`);
        }
